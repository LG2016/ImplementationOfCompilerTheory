...
class Data {
	...
	public static int[][] analysis = new int[68][37]; // LL(1)分析表  产生式左边有68不同的  产生式的predict集中有36+1(#)
	...
	public static void initialize() { // 初始化分隔符列表、保留字列表、大小写字母表、0-9数字表、1-9数字表
										//终极符表、非终结符符表、和SNL的规则
		
		separator = new ArrayList<String>();//分隔符列表
		reservedWord = new ArrayList<String>();//保留字列表
		...
		L = new ArrayList<Character>();//大小写列表
		D = new ArrayList<Character>();//0-9数字列表
		D1 = new ArrayList<Character>();//1-9数字列表
		...
		r = new Rule();//填充下标为2的规则
		r.A = "ProgramHead";
		r.B.add("program");
		r.B.add("ProgramName");
		rule.add(r);

		r = new Rule();//填充下标为3的规则
		r.A = "ProgramName";
		r.B.add("ID");
		rule.add(r);
